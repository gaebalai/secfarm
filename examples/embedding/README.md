...
|------|-------|---------------------|---------|-------------|
| `--dimensions` | `-d` | `SECFARM_EMBEDDING_DIMENSIONS` | 8 | Embedding dimensions |
| `--gemini-project` | - | `SECFARM_GEMINI_PROJECT` | - | Google Cloud project ID (required) |
| `--gemini-location` | - | `SECFARM_GEMINI_LOCATION` | us-central1 | Google Cloud location |
| `--show-vector` | `-v` | `SECFARM_SHOW_VECTOR` | false | Show embedding vectors |
| `--json` | `-j` | `SECFARM_OUTPUT_JSON` | false | Output in JSON format |
...
```bash
export SECFARM_GEMINI_PROJECT=your-project
export SECFARM_EMBEDDING_DIMENSIONS=256
