...
### 1. Run secfarm with the MCP server

From this directory:

```bash
cd examples/mcp-server
secfarm chat --mcp-config config.yaml --alert-id <your-alert-id>
```

Or from the repository root:

```bash
secfarm chat --mcp-config examples/mcp-server/config.yaml --alert-id <your-alert-id>
```
