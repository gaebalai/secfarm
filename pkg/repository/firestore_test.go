...
func TestFirestorePutMemory(t *testing.T) {
	...
	memory := &model.Memory{
		...
		Claim:     "테스트 테이블은 project.dataset.table에 저장되어 있다",
		QueryText: "테스트 쿼리",
		Embedding: testEmbedding,
		Score:     0.0,
		CreatedAt: time.Now(),
...
func TestFirestoreSearchMemories(t *testing.T) {
	...
	memory1 := &model.Memory{
		ID:        model.NewMemoryID(),
		Claim:     "감사 로그는 project.logs.audit에 저장되어 있다",
		QueryText: "감사 로그 위치",
		Embedding: embedding1,
		Score:     5.0,
		CreatedAt: now.Add(-3 * time.Hour),
	...
	memory2 := &model.Memory{
		ID:        model.NewMemoryID(),
		Claim:     "액세스 로그는 project.logs.access에 저장되어 있다",
		QueryText: "액세스 로그 위치",
		Embedding: embedding2,
		Score:     3.0,
		CreatedAt: now.Add(-2 * time.Hour),
	...
	memory3 := &model.Memory{
		ID:        model.NewMemoryID(),
		Claim:     "에러 로그는 project.logs.error에 저장되어 있다",
		QueryText: "에러 로그 위치",
		Embedding: embedding3,
		Score:     1.0,
		CreatedAt: now.Add(-1 * time.Hour),
