...
type Plan struct {
	...
	Steps       []Step    `json:"steps"`     // 실행 스텝
	GeneratedAt time.Time `json:"generated_at"`
}

// Step represents a single step in the investigation plan
type Step struct {
	ID          string     `json:"id"`          // 스텝 ID (예: "step_1")
	Description string     `json:"description"` // 스텝 설명
	Tools       []string   `json:"tools"`       // 예상 도구
	Expected    string     `json:"expected"`    // 기대되는 성과
...
type Reflection struct {
	...
	Insights    []string     `json:"insights"`     // 새로운 인사이트
	PlanUpdates []PlanUpdate `json:"plan_updates"` // 계획 업데이트
...
type PlanUpdate struct {
	...
	Step   Step       `json:"step,omitempty"`    // 스텝 정보 (add_step/update_step의 경우)
	StepID string     `json:"step_id,omitempty"` // 스텝 ID (cancel_step의 경우)
