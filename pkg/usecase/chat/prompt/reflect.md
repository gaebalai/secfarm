...
```json
{
  ...
  "step": {
    "id": "step_3a",
    "description": "과거의 유사 알림을 검색",
    "tools": ["search_alerts"],
    "expected": "유사한 알림 패턴과 대응 이력"
  }
}
```
...
```json
{
  ...
  "step": {
    "id": "step_4",
    "description": "더 넓은 기간(7일간)의 로그를 BigQuery에서 검색",
    "tools": ["bigquery_query"],
    "expected": "관련된 모든 로그 이벤트"
  }
}
```
...
{
  "achieved": false,
  "insights": ["도구가 존재하지 않아 이 조사를 실행할 수 없었습니다. 다른 접근 방식은 초기 계획에서 검토했어야 합니다."],
  "plan_updates": []
}
```
...
{
  "achieved": false,
  "insights": ["해당하는 로그 데이터를 찾을 수 없었습니다. 더 이상의 조사는 불가능합니다."],
  "plan_updates": []
}
```
...
{
  "achieved": false,
  "insights": ["API 에러가 발생했습니다. 시스템 제약으로 인해 이 정보 소스를 이용할 수 없습니다."],
  "plan_updates": []
}
```
